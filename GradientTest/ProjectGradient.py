...
class MyModel(nn.Module):
    ...
    def forward2(self):
        ...
        contrain = 0
        temp = torch.tensor([1 , 1 , 1] , dtype = torch.float64)
        for k in range(self.horizon):

            contrain += temp @ (self.states[k + 1].t() - self.A @ self.states[k].t() - self.B @ self.controls[k].t())
    ...
    def train(self):
        ...
        # for i in range(1):
            ...
            self.total_cost = self.forward1()
            self.total_cost.backward()

            print("self.states[i + 1].grad = " ,  self.states[1].grad)


            cost_grads = {name: param.grad.clone() for name, param in self.named_parameters() if param.grad }
            print("cost_grad = ", cost_grads)


            # self.zero_grad()
            # self.states[1].grad.zero_()
            # print("self.states[i + 1].grad = " ,  self.states[1].grad)

        #     self.constraint = self.forward2()
        #     self.constraint.backward()
        
        # # Store the gradients for the constraint function
        #     constraint_grads = {name: param.grad.clone() for name, param in self.named_parameters() if param.grad is not None}

            # print("self.states[i + 1].grad = " ,  self.states[i + 1].grad)
            # print("self.total_cost.grad = " , self.total_cost.grad)
            # print("after = " , self.states[i + 1].grad)
            print("cost_grads = " , cost_grads)
            print("constraint = " , constraint_grads)
            exit()
            ...
            with torch.no_grad():
                ...
                for i in range(self.horizon):
                    self.total_cost = self.forward1()
                    self.total_cost.backward()
                    temp1 = self.states[i + 1].grad.clone()
                    self.states[i + 1].grad.zero_()

                    self.constrain = self.forward2()
                    self.constrain.backward()
                    temp2 = self.states[i + 1].grad.clone()
                    self.states[i + 1].grad.zero_()
...
for i in range(1):
    ...
    model.total_cost = model.forward1()
    model.total_cost.backward()

    print("self.states[i + 1].grad = " ,  model.states[1].grad)
# model.zero_grad()
