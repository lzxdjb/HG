...
int main()
{
    ...
    SharedMatrix * h_A_dense = ( SharedMatrix*)malloc(sizeof(*h_A_dense));


    // --- Column-major ordering
  h_A_dense->row(0)[0] = 1.0f; h_A_dense->row(0)[1] = 0.0f; h_A_dense->row(0)[2] = 5.0f; h_A_dense->row(0)[3] = 0.0f;
h_A_dense->row(1)[0] = 4.0f; h_A_dense->row(1)[1] = 2.0f; h_A_dense->row(1)[2] = 0.0f; h_A_dense->row(1)[3] = 0.0f;
h_A_dense->row(2)[0] = 0.0f; h_A_dense->row(2)[1] = 3.0f; h_A_dense->row(2)[2] = 0.0f; h_A_dense->row(2)[3] = 9.0f;
h_A_dense->row(3)[0] = 0.0f; h_A_dense->row(3)[1] = 0.0f; h_A_dense->row(3)[2] = 7.0f; h_A_dense->row(3)[3] = 0.0f;

    //create device array and copy host to it
    SharedMatrix *d_A_dense;  gpuErrchk(cudaMalloc(&d_A_dense,  sizeof(SharedMatrix)));
    gpuErrchk(cudaMemcpy(d_A_dense, h_A_dense, sizeof(SharedMatrix), cudaMemcpyHostToDevice));
