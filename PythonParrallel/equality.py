...
######
    ...
    def getfinalmatrix(self):
        ...
        print("before final_matrix = \n" , self.final_matrix)
        print("before Lower = \n" , self.Lower)
        
        base = self.horizon * (self.state_shape + self.control_shape)

        for i in range(0 , (self.horizon - 1)* (self.state_shape + self.control_shape) , self.state_shape + self.control_shape):

            iterate = i / (self.state_shape + self.control_shape)
            # iterate += 1

            base = (self.horizon - iterate) * (self.state_shape + self.control_shape)

            for j in range(i):

                self.eliminate(i + j , i + j , i + (base - j) , i + j)

                self.eliminate(i , i , i + base +   self.state_shape , i)

                self.eliminate((i + 1) , (i + 1) ,  (i + 1) + base , (i + 1))
                self.eliminate((i + 1) , (i + 1) ,  (i + 1) + base + self.state_shape ,  (i + 1))

                self.eliminate((i + 2) , (i + 2) ,  (i + 2) + base , (i + 2))

                self.eliminate((i + 2) , (i + 2) ,  (i + 2) + base + 1 , (i + 2))
                self.eliminate((i + 2) , (i + 2) ,  (i + 2) + base + 2 , (i + 2))
                self.eliminate((i + 2) , (i + 2) ,  (i + 2) + base + 3 , (i + 2))

                self.eliminate((i + 3) , (i + 3) ,  i + base , (i + 3))
                self.eliminate((i + 3) , (i + 3) ,  i + base + 1 , (i + 3))

                self.eliminate((i + 4) , (i + 4) ,  i + base + 2 , (i + 4))

        # i = (self.horizon - 1) * (self.state_shape + self.control_shape)
